...
func _run() -> void:
	...
	assert(room.initial_enemies.size() >= 3 and room.reinforcement_enemies.size() >= 2)
	await _capture("task41_01_initial_wave_1920x1080.png", Vector2i(1920, 1080))
	_defeat_batch(room.initial_enemies)
	await process_frame
	assert(room.reinforcement_activated and not room.room_is_cleared)
	await _wait_frames(60)
	await _capture("task41_02_reinforcement_wave_1920x1080.png", Vector2i(1920, 1080))
	_defeat_batch(room.reinforcement_enemies)
	await process_frame
	assert(room.room_is_cleared and room.chest.visible and room.portal.locked)
	await _wait_frames(60)
	await _capture("task41_03_closed_chest_locked_portal_1920x1080.png", Vector2i(1920, 1080))
	_interact_at(room.chest)
	await process_frame
	assert(room.chest.consumed and room.portal.enabled and not room.portal.locked)
	await _capture("task41_04_open_chest_unlocked_portal_1920x1080.png", Vector2i(1920, 1080))
	...
	await _capture("task41_05_portal_to_route_1920x1080.png", Vector2i(1920, 1080))
	...
	await _capture("task41_06_single_shop_1366x768.png", Vector2i(1366, 768))
	var panel_sha := FileAccess.get_sha256("res://docs/agent_tasks/evidence/task41/screenshots/task41_06_single_shop_1366x768.png")
	...
	await _capture("task41_07_shop_exit_f_prompt_1366x768.png", Vector2i(1366, 768))
	assert(FileAccess.get_sha256("res://docs/agent_tasks/evidence/task41/screenshots/task41_07_shop_exit_f_prompt_1366x768.png") != panel_sha)
	...
	await _capture("task41_08_boss_purple_outline_2560x1440.png", Vector2i(2560, 1440))
	...
	await _capture("task41_09_boss_low_projectile_jump_path_2560x1440.png", Vector2i(2560, 1440))
	_defeat_batch(room.enemies)
	await process_frame
	assert(room.room_is_cleared and room.chest.visible and room.portal == null)
	await _capture("task41_10_settlement_chest_2560x1440.png", Vector2i(2560, 1440))
	...
	assert(final.route.completed_combat_rooms == 6 and final.route.shop_visits == 1)
	assert(final.economy.balance == balance_before)
	await _capture("task41_11_results_2560x1440.png", Vector2i(2560, 1440))
