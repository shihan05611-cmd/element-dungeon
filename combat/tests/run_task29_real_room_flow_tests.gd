...
func _run() -> void:
	...
	_expect(coordinator.choose_route(&"route_01_pressure").accepted, "first route selects pressure target")
	_expect(await _wait_for_room(coordinator, &"combat_02_pressure"), "pressure PackedScene becomes active")
	...
	_expect(coordinator.choose_route(&"route_02_risk").accepted, "second route selects risk target")
	_expect(await _wait_for_room(coordinator, &"combat_05_risk"), "risk PackedScene becomes active")
	_record_room(coordinator, scene_paths, room_instances)
	...
	_expect_eq(scene_paths.size(), 6, "six scene activations recorded by runner")
	_expect_eq(room_instances.size(), 6, "six room instance activations recorded by runner")
	_expect_eq(_unique_int_count(room_instances), 6, "every combat uses a different RunRoomInstance")
	_expect(_unique_string_count(scene_paths) >= 4, "run observes at least four PackedScene templates")
...
func _finish() -> void:
	if _failures.is_empty():
		print("TASK 29 REAL ROOM FLOW TESTS PASSED: %d tests, %d assertions" % [
			_tests,
			_assertions,
		])
		quit(0)
	else:
		printerr("TASK 29 REAL ROOM FLOW TESTS FAILED: %d failures / %d assertions" % [
			_failures.size(),
			_assertions,
		])
