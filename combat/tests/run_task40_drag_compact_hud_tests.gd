...
func _finish() -> void:
	if _failures.is_empty():
		print("TASK 40 DRAG COMPACT HUD TESTS PASSED: %d tests, %d assertions" % [_tests, _assertions])
		quit(0)
	else:
		printerr("TASK 40 DRAG COMPACT HUD TESTS FAILED: %d failures / %d assertions" % [_failures.size(), _assertions])
