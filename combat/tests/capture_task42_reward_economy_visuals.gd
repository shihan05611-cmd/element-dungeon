...
func _capture_safe_run() -> void:
	...
	await _boot(&"task31_safe")
	await _record_and_finish_normal_room()
	assert(await _wait_phase(RunPhase.ROUTE_CHOICE))
	assert(_coordinator.choose_route(&"route_01_swarm").accepted)
	assert(await _wait_combat(&"combat_02_swarm"))
	await _record_and_finish_normal_room()
	assert(await _wait_combat(&"combat_03_layer_elite"))
	await _record_and_finish_normal_room()
	assert(await _wait_phase(RunPhase.SHOP))
	assert(await _wait_until(func() -> bool: return _coordinator.active_shop_room != null, 180))
	_assert_shop_authority(&"task31_safe", 495, 0, 0, 0, 495, 150, [&"elemental_laser", &"elemental_fury"])
	...
	await _upgrade(&"element_bolt", 2)
	await _upgrade(&"element_bolt", 3)
	await _reset(&"element_bolt")
	await _upgrade(&"element_bolt", 2)
	await _upgrade(&"element_bolt", 3)
	_assert_shop_authority(&"task31_safe", 495, 300, 300, 105, 0, 150, [&"elemental_laser", &"elemental_fury"])
	_assert_levels_and_loadout(true)
	...
	await _record_and_finish_normal_room()
	assert(await _wait_phase(RunPhase.ROUTE_CHOICE))
	assert(_coordinator.choose_route(&"route_02_stable").accepted)
	assert(await _wait_combat(&"combat_05_stable"))
	await _record_and_finish_normal_room()
	...
	_assert_result_authority(final, &"task31_safe", 895, 300, 300, 105, 400, 300, [&"elemental_laser", &"elemental_fury", &"element_reclaim"])
	_assert_levels_and_loadout(true)
...
func _capture_risk_run() -> void:
	root.size = Vector2i(1920, 1080)
	await _boot(&"task31_risk")
	await _record_and_finish_normal_room()
	assert(await _wait_phase(RunPhase.ROUTE_CHOICE))
	assert(_coordinator.choose_route(&"route_01_pressure").accepted)
	assert(await _wait_combat(&"combat_02_pressure"))
	await _record_and_finish_normal_room()
	assert(await _wait_combat(&"combat_03_layer_elite"))
	await _record_and_finish_normal_room()
	assert(await _wait_phase(RunPhase.SHOP))
	...
	await _upgrade(&"elemental_laser", 2)
	_assert_shop_authority(&"task31_risk", 665, 390, 115, 0, 160, 300, [&"elemental_laser"])
	_assert_levels_and_loadout(false)
	await _leave_physical_shop()
	assert(await _wait_combat(&"combat_04_validation"))
	await _record_and_finish_normal_room()
	assert(await _wait_phase(RunPhase.ROUTE_CHOICE))
	assert(_coordinator.choose_route(&"route_02_risk").accepted)
	assert(await _wait_combat(&"combat_05_risk"))
	await _record_and_finish_normal_room()
	assert(await _wait_combat(&"combat_06_final_boss"))
	await _finish_boss()
	assert(await _wait_phase(RunPhase.RUN_COMPLETE))
	var final := _coordinator.current_snapshot()
	_assert_result_authority(final, &"task31_risk", 1150, 390, 115, 0, 645, 450, [&"elemental_laser", &"elemental_fury"])
	_assert_levels_and_loadout(false)
	root.size = Vector2i(2560, 1440)
...
func _assert_shop_authority(run_id: StringName, earned: int, purchases: int, upgrades: int, refunded: int, balance: int, chest_dust: int, chest_skills: Array[StringName]) -> void:
	...
	assert(snapshot.route.shop_visits == 1 and snapshot.route.completed_combat_rooms == 3)
...
func _assert_result_authority(final: RunSnapshot, run_id: StringName, earned: int, purchases: int, upgrades: int, refunded: int, balance: int, chest_dust: int, chest_skills: Array[StringName]) -> void:
	...
	assert(final.route.completed_combat_rooms == 6 and final.route.shop_visits == 1 and final.route.route_choices == 2)
...
func _assert_levels_and_loadout(safe: bool) -> void:
	var snapshot := _coordinator.current_snapshot()
	assert(snapshot.loadout.entries.size() == 7)
	assert(snapshot.loadout.get_skill_id(SkillSlotIds.ACTIVE_1) == &"element_bolt")
	assert(snapshot.skills.progress_for(&"element_bolt").level == (3 if safe else 1))
	if safe:
		assert(snapshot.loadout.get_skill_id(SkillSlotIds.ACTIVE_2).is_empty())
		assert(snapshot.loadout.get_skill_id(SkillSlotIds.ACTIVE_3).is_empty())
	else:
		assert(snapshot.loadout.get_skill_id(SkillSlotIds.ACTIVE_2) == &"elemental_laser")
		assert(snapshot.loadout.get_skill_id(SkillSlotIds.ACTIVE_3) == &"element_reclaim")
		assert(snapshot.skills.progress_for(&"elemental_laser").level == 2)
		assert(snapshot.skills.progress_for(&"element_reclaim").level == 2)
	for index: int in PASSIVE_IDS.size():
...
func _capture(file_name: String, expected_size: Vector2i) -> void:
	var snapshot := _coordinator.current_snapshot()
	assert(snapshot.route.run_id == &"task31_safe" or snapshot.route.run_id == &"task31_risk")
