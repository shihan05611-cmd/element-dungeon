...
const EVIDENCE_DIR := "res://docs/agent_tasks/evidence/task20"
...
func _run() -> void:
	...
	if directory_error != OK:
		printerr("TASK 20 evidence directory failed: %d" % directory_error)
		quit(1)
		return
	...
	await _settle()
	await _capture("01_hud_1920x1080_main.png")

	root.size = Vector2i(2560, 1440)
	await _settle()
	await _capture("01b_hud_2560x1440_scale.png")

	root.size = Vector2i(3840, 2160)
	await _settle()
	await _capture("01c_hud_3840x2160_scale.png")

	root.size = Vector2i(1152, 648)
	await _settle()
	await _capture("01d_hud_1152x648_logic.png")

	root.size = Vector2i(900, 540)
	await _settle()
	await _capture("02_hud_900x540.png")
	...
	transients[SkillSlotIds.ACTIVE_1] = {"text": "2.4s", "tone": &"cooldown", "until": until}
	transients[SkillSlotIds.ACTIVE_2] = {"text": "◇ 能量", "tone": &"energy", "until": until}
	transients[SkillSlotIds.ACTIVE_3] = {"text": "▰ 忙", "tone": &"busy", "until": until}
	...
	cooldown_mask.set_anchor(SIDE_TOP, 0.42)
	cooldown_label.visible = true
	cooldown_label.text = "2.4s"
	_hud.call("_show_feedback", "当前忙碌 · 动作结束后重试", &"busy", 5.0)
	await _settle()
	await _capture("03_states_cooldown_energy_busy_failure.png")
	...
	_hud.set_authoritative_target(_enemy)
	await _settle()
	await _capture("04_target_follow.png")

	_enemy.position.x = 1800.0
	await _settle()
	await _capture("05_target_fallback.png")
	_enemy.position.x = 820.0
	_hud.set_authoritative_target(_enemy)

	_overlay.show_reward(_offer(3, false))
	await _settle()
	await _capture("06_reward_three.png")
	_overlay.hide_overlay()

	_overlay.show_reward(_offer(2, false))
	await _settle()
	await _capture("07_reward_two.png")
	_overlay.hide_overlay()

	_overlay.show_reward(_offer(1, false))
	await _settle()
	await _capture("08_reward_one_explicit_confirm.png")
	_overlay.hide_overlay()

	_overlay.show_reward(_offer(3, true))
	await _settle()
	await _capture("09_reward_long_copy.png")
	_overlay.hide_overlay()

	_hud.set_colorblind_mode(true)
	_enemy.element_carrier.set_amounts_silent(4, 1)
	_hud.set_authoritative_target(_enemy)
	await _settle()
	await _capture("10_colorblind.png")
	_hud.set_colorblind_mode(false)

	_hud.set_reduced_motion(true)
	_hud.call("_show_feedback", "减少动态 · 元素锁定与失败语义保持", &"lock", 5.0)
	await _settle()
	await _capture("11_reduced_motion.png")
	_hud.set_reduced_motion(false)

	await _stage_task18_vfx()
	await _settle()
	await _capture("12_task18_vfx_unobscured.png")

	print("TASK 20 VISUAL CAPTURE COMPLETE: 15 screenshots")
	quit(0)


func _stage_task18_vfx() -> void:
	_overlay.hide_overlay()
	_enemy.position.x = 520.0
	...
	if not fury_attempt.accepted:
		printerr("TASK 20 Fury fixture rejected: " + String(fury_attempt.reason_name()))
	...
	if not laser_attempt.accepted:
		printerr("TASK 20 Laser fixture rejected: " + String(laser_attempt.reason_name()))
	_player.skill_executor.advance(0.0)
...
func _equip_four() -> void:
	...
	if not result.accepted:
		printerr("TASK 20 four-skill fixture rejected: " + String(result.detail))
...
func _equip_one(skill_id: StringName) -> void:
	...
	var result := _host.runtime_loadout.try_replace_snapshot(RuntimeLoadoutSnapshot.new(entries, current.revision))
	if not result.accepted:
		printerr("TASK 20 skill fixture rejected: " + String(result.detail))
...
func _set_element(element_id: StringName) -> void:
	...
	if result == null or not result.accepted:
		printerr("TASK 20 element fixture rejected: " + String(element_id))
...
func _offer(count: int, long_copy: bool) -> RewardOffer:
	...
	for index: int in count:
		...
		options.append(RewardOption.new(
			StringName("task20_visual_option_%d_%d" % [count, index]),
			RewardType.SKILL,
			ids[index],
			display_name,
			description
	...
	return RewardOffer.new(
		StringName("task20_visual_offer_%d_%s" % [count, "long" if long_copy else "normal"]),
		&"task20_visual_room",
		RewardType.SKILL,
		31 + count,
		options
...
func _capture(file_name: String) -> void:
	...
	if result != OK:
		printerr("TASK 20 capture failed: %s (%d)" % [path, result])
		quit(1)
	print("CAPTURED " + path)
