...
func _test_safe_run() -> void:
	...
	_expect_eq(_unique_string_count(final.route.activated_scene_paths), 3, "safe route truthfully uses Battle01, Battle02, and Boss templates")
...
func _test_risk_run() -> void:
	...
	_expect_eq(_unique_string_count(final.route.activated_scene_paths), 3, "second run uses Battle01, Battle02, and Boss templates")
	var combined := _safe_scene_paths.duplicate()
	combined.append_array(final.route.activated_scene_paths)
	_expect_eq(_unique_string_count(combined), 3, "both route-free runs cover the fixed three templates")
...
func _finish() -> void:
	quit(_harness.report("TASK 31 FULL RUN E2E TESTS"))
