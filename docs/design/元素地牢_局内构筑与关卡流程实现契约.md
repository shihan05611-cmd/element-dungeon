...
Task42 对普通宝箱与当前经济节奏的最终冻结为：奖励类型使用稳定 `run_id + room_id` 哈希最低位，技能分支再用移除该位后的稳定值从排序后的未拥有 `reward_pool` 候选中取项，因此保持稳定 `50%` 技能 / `50%` `150` 梦尘且不再偏向偶数候选索引。safe/risk 五个普通战基础总收入分别为 `595 / 700`，到唯一商店前分别为 `345 / 365`；这些是当前原型节奏，不代表敌人强度、房间美术、长期 meta 或完整商业平衡已经定稿。

固定完整局事实为：`task31_safe` 获得 `300` 宝箱梦尘与 `elemental_laser → elemental_fury → element_reclaim`，最终账本 `895 / 300 / 300 / 105 / 400`（收入/购买/升级/返还/余额）；`task31_risk` 获得 `450` 宝箱梦尘与 `elemental_laser → elemental_fury`，最终账本 `1150 / 390 / 115 / 0 / 645`。
...
| 免费技能/遗物奖励三选一 | 战 1～5 实体宝箱：稳定 50% 未拥有 `reward_pool` 技能 / 50% 150 梦尘 | 移除正式 `REWARD` 页面；类型位与技能索引位解耦，类型化结果原子进入技能库或钱包 | 宝箱一次性、重放、分布与经济守恒 runner |
...
旧 `REWARD` 页面不再出现在正式路径。战 1～5 的实体宝箱仍处于 `COMBAT`，以稳定哈希最低位决定 50% 未拥有技能 / 50% 150 梦尘；技能分支使用移除最低位后的稳定值选择排序候选。宝箱和门不是新 `RunPhase`。
...
## 13. Task43：获得即装配、清场配装与房间收尾合同

### 13.1 获得技能的原子自动装配

- 普通宝箱获得新技能与商店购买新技能时，先使用现有 `RuntimeSkillLoadout` 校验同类型第一个空槽；主动固定按 `A1 → A2 → A3`，被动固定按 `P1 → P2 → P3 → P4`。
- 校验成功后，拥有技能、购买扣款（若有）、RunSnapshot 与 RuntimeLoadout 作为同一条获得命令发布；run revision 只推进一次，确有自动装配时 loadout revision 只推进一次。
- 没有 Runtime port、没有合法候选或同类型已满时，技能仍正常进入拥有列表，但不覆盖、交换、升级或重复装配任何槽位。
- replay、重复宝箱和重复购买继续沿用既有命令记录，不再次推进 revision。Task42 的 `50% 技能 / 50% 150 梦尘`、房间梦尘、单商店和两条固定整局经济完全不变。

### 13.2 正式战斗中的配装边界

- 正式 `COMBAT` 按 `L` 可直接打开战斗配装页，不创建或复用 `ShopDraft`。
- 当前房仍有活敌时页面只读，整块技能面板变暗且技能/槽位按钮 disabled，不再用文字提示；点击、拖拽、卸下均不调用权威事务，钱包、七槽、run/loadout revision 保持不变。
- 只有当前 `RunRoomInstance.room_is_cleared == true` 且 Host 确认全部敌人已阵亡后，Coordinator 才转发点击、拖拽、换位与卸下到 RunSession 的一次原子替换；同映射为 no-op，拒绝后恢复现场快照。
- Host/Room 遍历敌人引用前先检查 `is_instance_valid`；已释放的正式敌人按已阵亡处理。

### 13.3 正式敌人、宝箱与平台

- 仅通过 `configure_run_spawn` 创建的正式敌人，在同步发出 `enemy_defeated` 并完成既有结算观察后 deferred `queue_free`；下一帧节点失效，Boss 不再产生新弹体。TestRoom 的历史重置夹具不受影响。
- 初始波死亡仍立即激活预实例化援军；两波全部结束仍只生成一次宝箱和传送门。
- 普通/Boss 结算宝箱根位置固定为 `y=501`，在现有 `0.46` 缩放下 alpha 可见底边落到 `540±2px`，不依赖尸体或动态物理。
- `room_arena_platforms.tscn` 的 `LowerPlatform` 中心为 `y=442`，使 `combat_02_pressure` 与 `combat_03_layer_elite` 都能由现有 Player 跳跃/重力真实到达；不修改玩家参数或增加攀爬逻辑。

### 13.4 验收事实

- Task43 专项为 `4 tests / 125 assertions`；Task31/32/40/41 直接 runner 均通过。
- 固定 Task31 safe/risk 身份、六战/两路线/单商店、Task42 经济和七槽生命周期保持强断言。
- 从头正式集合 `34/34`，合计 `319 tests / 6889 assertions`；RunGame/TestRoom 双 180 帧 smoke 通过。
- 恰好五张实际 Viewport 画面分别证明活敌只读、清场可写、无尸体且宝箱贴地、两个风险房模板真实跳上首平台。
