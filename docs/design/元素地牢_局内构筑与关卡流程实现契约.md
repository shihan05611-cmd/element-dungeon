...
> 文档性质：任务 27～31 的实现合同与验收基线  
> 状态：任务 26 执行侧冻结稿，待中枢 Review  
> 形成日期：2026-08-05  
...
| 免费技能/遗物奖励三选一 | 战斗结算梦尘；技能在商店购买 | 移除正式 `REWARD` 领取阶段；保留旧类型只供历史代码加载 | 全局流程没有免费奖励页面 |
...
| 每 3 房固定商店，第 6 房后商店结束 | 房 1/3/5 后整备，第 6 房 Boss 后直结算 | 流程由静态节点图驱动，不再由取模决定 | 六节点自动化和真实主入口完整一局 |
...
| 战 1 | `combat_01_entry` | COMBAT | 入口战；基础操作和水火反应 | `shop_01_early` |
| 整备 1 | `shop_01_early` | SHOP | 第一购买机会；全量购买/升级/重置/装配 | `route_01_first_branch` |
...
| 战 3 | `combat_03_layer_elite` | COMBAT | 第一层层主精英；更强现有敌人占位 | `shop_02_mid` |
| 整备 2 | `shop_02_mid` | SHOP | 中段完整整备 | `combat_04_validation` |
| 战 4 | `combat_04_validation` | COMBAT | 构筑验证战；强度回落后组合测试 | `route_02_second_branch` |
| 路线 2A | `route_02_stable` → `combat_05_stable` | ROUTE→COMBAT | 稳妥收束、标准梦尘 | `shop_03_preboss` |
| 路线 2B | `route_02_risk` → `combat_05_risk` | ROUTE→COMBAT | 更多/更强敌人、狭长环境、额外梦尘 | `shop_03_preboss` |
| 整备 3 | `shop_03_preboss` | SHOP | 终战前最后完整消费与配装机会 | `combat_06_final_boss` |
| 战 6 | `combat_06_final_boss` | BOSS | 强化现有敌人的占位最终 Boss；奖励梦尘为 0 | `RUN_COMPLETE` |
...
旧 `REWARD` 不再出现在正式路径。`ROOM_RESOLUTION` 只原子结算梦尘、战绩和下一节点，不生成免费内容候选。
...
Boss 的 `ROOM_RESOLUTION` 在同一房间完成命令中直接形成 `RUN_COMPLETE` 结果，不短暂进入路线、奖励或商店。
...
区别只在静态库存/候选。`shop_01_early` 必须至少有一个买得起的主动候选，且房 1 的保底梦尘足以完成至少一笔有效购买。离店不提交属性、不二次提交配装，也不自动恢复全部生命或 SP；受控恢复若需要，作为任务 31 的显式节点数值配置。
...
八个可能的战斗配置引用上述模板并指定不同的敌人数、出生点、生命/防御配置、梦尘和标签。最终 Boss 可复用现有敌人场景并提高明确配置，不要求新美术、招式或多阶段机制。

### 7.5 职责边界

| 对象 | 唯一职责 | 明确禁止 |
| --- | --- | --- |
| `RunSession` | 经济、拥有、等级、七槽协调、节点阶段、结果的领域权威 | Node、PackedScene 实例、UI、直接改 Player |
...
| `RunRoomInstance` | 几何、出生点、按静态配置生成/暴露本房敌人 | 保存跨房状态、发奖励、决定下一房 |
...
| 14 | Boss 后直结算，无无效梦尘/成长奖励 | 6、8.3 | 29、31 |
