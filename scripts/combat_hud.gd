...
func _refresh_slot(slot_id: StringName) -> void:
	...
	var meta := view["meta"] as Label
	var is_passive_slot := slot_id == SkillSlotIds.PASSIVE_1
	(view["slot"] as Label).text = String(slot_id).to_upper()
	if skill == null:
		icon.texture = null
		name_label.text = "空槽"
		policy.text = "— 未装备 —"
	...
	policy.text = _skill_policy_badge(skill)
	...
	if not castable_here:
		state.text = "被动" if skill.is_passive_skill() else "不可释放"
		state.add_theme_color_override(&"font_color", UI.WARNING)
		meta.text = "持续生效 · 无键帽/能量/冷却"
		return
	state.text = _availability_text(skill)
...
func _availability_color(text_value: String) -> Color:
	if text_value == "可用":
		return UI.SUCCESS
	if text_value.begins_with("冷却"):
		return UI.COOLDOWN
	if text_value == "忙碌" or text_value == "施放中":
		return UI.BUSY
	return UI.ERROR
...
func _bind_ui_refs() -> void:
	...
	element_swatch = $Root/StatusPanel/Margin/Status/TitleRow/ElementBadge/BadgeMargin/BadgeRow/ElementSwatch
	element_text = $Root/StatusPanel/Margin/Status/TitleRow/ElementBadge/BadgeMargin/BadgeRow/ElementText
	...
	for slot_id: StringName in SLOT_ORDER:
		var card := $Root/SkillPanel/Margin/Skills/SlotRow.get_node(String(slot_id)) as PanelContainer
		_slot_views[slot_id] = {
			"panel": card,
...
func _build_status_panel(parent: Control) -> void:
	...
	panel.size = Vector2(304, 144)
	panel.add_theme_stylebox_override(&"panel", UI.panel())
	parent.add_child(panel)
	var margin := _margin("Margin", 12, 10)
	...
	var title := _make_label("Title", "法雅雅", 18, UI.TEXT)
	...
	box.add_child(_bar_row("HealthRow", "生命", "HealthBar", "HealthValue", Color("dc4658")))
	var low := _make_label("LowHealth", "低生命 · 注意防护", 12, UI.ERROR)
	low.visible = false
	low.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT
	box.add_child(low)
	box.add_child(_bar_row("EnergyRow", "能量", "EnergyBar", "EnergyValue", Color("289dcf")))
...
func _build_skill_panel(parent: Control) -> void:
	...
	panel.position = Vector2(-344, -132)
	panel.size = Vector2(688, 112)
	panel.add_theme_stylebox_override(&"panel", UI.panel(Color(0.035, 0.055, 0.09, 0.97)))
	parent.add_child(panel)
	var margin := _margin("Margin", 10, 8)
	panel.add_child(margin)
	...
	for slot_id: StringName in SLOT_ORDER:
		row.add_child(_build_hud_slot(slot_id))
...
func _build_feedback_panel(parent: Control) -> void:
	...
	panel.size = Vector2(420, 48)
	panel.add_theme_stylebox_override(&"panel", UI.panel(UI.SURFACE_RAISED, UI.BORDER_FOCUS, 7, 1))
	parent.add_child(panel)
	var margin := _margin("Margin", 12, 7)
	panel.add_child(margin)
	var text := _make_label("Text", "反馈", 15, UI.TEXT)
...
func _build_help_panel(parent: Control) -> void:
	...
	panel.set_anchors_preset(Control.PRESET_CENTER_TOP)
	panel.position = Vector2(-250, 74)
	panel.size = Vector2(500, 36)
